...
export const register = async (userName, email, age, password) => {
  if (
    userName === undefined ||
    email === undefined ||
    age === undefined ||
...
export const login = async (userName, password) => {
  ...
  if (!fetch_User) throw `Either the userName or password is invalid`;

  let check_password = await bcrypt.compare(password, fetch_User.password);
  if (!check_password) throw `Either the userName or password is invalid`;
